...
<div id="filtros" class="clearfix">
    <form>
        <div class="small-4 columns">
            <label>Fecha
                <select name="fecha" id="fecha" class="text-center">
                    <?php foreach($data["fechas"] as $fecha):?>
                        <?php if($fecha->field_fecha_abierta['und'][0]['value']==1):?>
                            <option <?php echo ($fecha->nid==$data["alineacion"]->field_fecha_torneo['und'][0]['target_id'])?"selected":""?> class="text-center" value="<?php echo $fecha->nid?>"><?php echo $fecha->title?></option>
                        <?php endif?>
                    <?php endforeach?>
                </select>
            </label>
        ...
        <div class="small-4 columns">
            <label>Formaci&oacute;n
                <select name="formacion" id="formacion" class="text-center">
    ...
    <div id="suplentes">
        <?php if(isset($data["alineacion"]->field_suplentes['und'])):?>
            <?php foreach($data["alineacion"]->field_suplentes['und'] as $player):?>
                <img src="/sites/all/themes/soydt_foundation/images/players/player.png">
            <?php endforeach?>
        <?php endif?>
    </div>
</div>
